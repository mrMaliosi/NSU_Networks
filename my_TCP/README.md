...
## Сборка и запуск
**1. Собираем и запускаем сервер на порту**
   ```bash
   go build -o server/server.exe server/server.go; ./server/server.exe <порт>
   ```
**2. Запуск клиента с файлом для передачи**
```bash
go run client/client.go localhost <порт на котором расположен сервер> <путь до файла>
```
## Формулировка задачи
### Цель 
Вам необходимо разработать протокол передачи произвольного файла с одного компьютера на другой, и написать клиент и сервер, реализующие этот протокол. 
